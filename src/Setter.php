...
class Setter extends AbstractSetter {
  ...
  //private string renameFilename|null|**Deprecated**. Use renameFile instead.
  //private string renameFile|null|A function that is invoked before the file is uploaded to the server and renames the file. This function gets the File as argument and can use the file.name. The actual name of the file that gets used during the upload can be accessed through file.upload.filename.
  ...
  * @return object
  ...
  public function __call(string $name, array $arguments): object {
    if (strpos($name, 'set')===0) {
      $var = lcfirst(str_replace('set', '', $name, 1));
      if (in_array($var, $this->_allowedOptions)) {
        $unionTypes = array_map(function($type) {
            return $type->getName();
        }, $rp->getType()->getTypes());
        if (in_array(gettype($arguments[0]), $unionTypes)) {
